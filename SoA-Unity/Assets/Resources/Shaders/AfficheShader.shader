		...
		SubShader
	{
		Pass
		{
			...
			struct v2f
			{
				...
				float3 light_pos : POS;//en fait ce que l'on veut ici ^^
				float3 origin_pos : ORIGIN;
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				//voir rendu diffuse avec transparence pour lumiere
				...
				//Point Light calcul
				float3 vertexToLightSource = i.light_pos.xyz - i.origin_pos.xyz;
				...
				float3 diffuseReflection = _LightIntensity * attenuation * float3(1.0f, 1.0f, 1.0f);// *max(0.0, dot(normal, lightDirection));
				...
				sample *= light;//(_AmbientColor + light + specular);// +rim);
				sample.a = _Transparency;
				//sample = float4(normal.xyz, 1.0f);
				return sample;
			}
			ENDCG
		}
