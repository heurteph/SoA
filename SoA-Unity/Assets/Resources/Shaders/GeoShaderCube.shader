Shader "Shaders/GeoShaderCube"
{
	Properties{
		...
		void geo(triangle vertexOutput IN[3], inout TriangleStream<vertexOutput> triStream) {
			...
			float speed = 40.0f;
			//toujours positif
			float time = (sin(_Time.x*speed) + 1.0f)/2.0f;

			vertexOutput v = IN[0];
			vNormal = v.normal;
			vTangent = v.tangent;
			vBinormal = cross(vNormal, vTangent) * vTangent.w;
			...
			v.vertex += float4(normal*time * magnitude, 0.0f);
		...
		fixed4 frag(vertexOutput im) : COLOR{
			...
			return float4((_SinTime.y+1.0f)/2.0f, (_SinTime.y + 1.0f) / 2.0f, (_SinTime.y + 1.0f) / 2.0f,1.0f);
		}
		ENDCG
	}
	}
