					...
					if (_StateBlur){
						for (i = 0; i < 9; i++) {
							...
							if (_StateChromatique) {
								/*
								//1
								sampleTex[i].r = tex2D(_MainTex, float2(im.uv.x + (_OffsetColor.x / width), im.uv.y)).r;
								//0
								sampleTex[i].g = tex2D(_MainTex, float2(im.uv.x + (_OffsetColor.y / width), im.uv.y)).r;
								//-1
								sampleTex[i].b = tex2D(_MainTex, float2(im.uv.x + (_OffsetColor.z / width), im.uv.y)).r;
								*/
